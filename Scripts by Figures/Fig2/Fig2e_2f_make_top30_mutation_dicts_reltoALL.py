...
mutation_dict_2_top15 = {}
mutation_dict_18_top15 = {}
...
for guide in INDEL_DICT:
    ...
    for repl in INDEL_DICT[guide]:
        ...
        if 'day2' in INDEL_DICT[guide][repl][exp_type_plt]:
            ...
            for mutation in mutation_dict_2[guide]:
                ...
                total_count_v = (all_count[guide][repl])
                if total_count_v == 0:
                    mutation_dict_2[guide][mutation][repl] = 0
                else:
    ...
    for mut_item in sorted(Freq_dict.items(), key=lambda t: t[1][0], reverse=True):
        ...
        if x < min(top_x, len_mutations):
            x += 1
            if not guide in mutation_dict_2_top15:
                mutation_dict_2_top15[guide] = {}
            mutation_dict_2_top15[guide][mutation] = mutation_dict_2[guide][mutation]
            x_list.append(x)
            y_list.append(y)
    ...
    for repl in INDEL_DICT[guide]:
        ...
        if 'day2' in INDEL_DICT[guide][repl][exp_type_plt]:
            ...
            for mutation in mutation_dict_18[guide]:
                ...
                # used either all_count or total_count as normalization - total count is more robust but makes less efficient guides less important
                total_count_v = (all_count_18[guide][repl])/3.6
                if total_count_v == 0:
                    mutation_dict_18[guide][mutation][repl] = 0
                else:
    ...
    for mut_item in sorted(Freq_dict.items(), key=lambda t: t[1][0], reverse=True):
        ...
        if x < min(top_x, len_mutations):
            x += 1
            if not guide in mutation_dict_18_top15:
                mutation_dict_18_top15[guide] = {}
            mutation_dict_18_top15[guide][mutation] = mutation_dict_18[guide][mutation]
            y_list_18.append(y_18)
            yerr_list_18.append(yerr_18)

pickle.dump(mutation_dict_2,open('mutations_dict_2-pos21-_g-m-r_relALL_n.sav','wb'))
pickle.dump(mutation_dict_18,open('mutations_dict_18-pos21-_g-m-r_relALL_n.sav','wb'))
pickle.dump(mutation_dict_2_top15,open('mutations_dict_2_top30_relALL_n.sav','wb'))
pickle.dump(mutation_dict_18_top15,open('mutations_dict_18_top30_relALL_n.sav','wb'))
