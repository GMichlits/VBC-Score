...
\f0\fs24 \cf0 INSTRUCTIONS for use of generate_VBCscore scripts:\
\
current version:\
for python2: generate_VBCscore_v0.0_py2.py\
\pard\tx720\tx1440\tx2160\tx2880\tx3600\tx4320\tx5040\tx5760\tx6480\tx7200\tx7920\tx8640\pardirnatural
\cf0 for python3: generate_VBCscore_v0.0_py3.py\
\
execution:\
copy the files \
...
execute the script (e.g. on mac in terminal by typing \'93python generate_VBCscore_v0.0_py2.py\'94)\
...
CTGGGCAGAGTTCTGGAAATT / \cf2 C\cf0 AGTGGAAT so the nucleotide C is considered the \'93cut site\'94, The 10bp upstream and downstream are added to give the 21bp range. Phylogenetic scores can be accessed via the UCSC genome browser.\
\
...
\
column [5] - distance: is the relative position (ranging from 0 to 1) of the cute site along the mRNA sequence (5\'92 to 3\'92 oritenation with 0 being the start  codon and 1 the stop codon).\
\
...
\
column [10] - AAcons7: If a score for phylogenetic conservation that is based on amino acid alignments. The score is to a large extend redundant to phylo21 and phast21 but has shown higher predictive power in our hands. Since derivation of the score can be challenging (for an instruction see Online methods \'93Generation of aa conservation score\'94 of associated publication) it is also possible to calculate VBC-Scores without AAcons7 input, in that case the median AAcons7 value of sgRNAs will be used for the purpose of calculating a VBC-Score.\
\
column [11] - AA_before_cut. The 6 amino acids upstream of the CRISPR cut site. (or less amino acids if there are for example less aa upstream because the cut is close to the start codon). For the codon \'93CAT\'94 in the sequence NNNCATNNN the cuts N / C, C / A and C / T belong to the amino acid histidin codon \'93CAT\'94. A CRISPR Cut between T / N would already refer to a cut \'93at\'94 the next amino acid. Use the one letter amino acid code. (sequence N to C terminal - or 5\'92 to 3\'92)\
\
column [12] - AA_at_cut. see definition before (always 1 amino acid) (sequence N to C terminal - or 5\'92 to 3\'92)\
\
column [13] - AA_after_cut. see definition before give 6 amino acids at most (sequence N to C terminal - or 5\'92 to 3\'92)\
\
...
\
column [14] - Doench 2016: The Doench2016 sgRNA score \'93Rule Set 2\'94.\
\
